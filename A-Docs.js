...
/*
  ...
  2. Fungsi yang Memanggil Fungsi Lain
     ...
     - containOneOf (memanggil: flat, getOptions, includes, isTypeOf, trim)
     - endWith (memanggil: flat, getOptions)
     - getA1NByName (memanggil: toA1Notation)
     - getCurrentDate (memanggil: getMonth)
     ...
     - isFalsy (memanggil: flat, getOptions)
     - isObject (memanggil: flat)
     - isSame (memanggil: flat)
     - isTruthy (memanggil: flat, getOptions)
     - isTypeOf (memanggil: flat, getOptions)
     - isYear (memanggil: between)
     - lazyWrap (memanggil: isIterable, wrap)
     - lowerThan (memanggil: flat, getOptions)
     - max (memanggil: flat)
     - normalizeFromCamelCase (memanggil: capitalize)
     - notSameWith (memanggil: flat, includes)
     - sameWith (memanggil: flat, getOptions, isObject)
     - show (memanggil: isObject, ui)
     - subtract (memanggil: flat, getOptions)
     - sum (memanggil: flat, isTypeOf, unique)
     - syncProtectionEditors (memanggil: batchUpdate, lazyWrap, retry)
     - syncProtectionEditorsBetweenSheet (memanggil: batchUpdate, includes, retry)
  ...
  3. Fungsi yang Dipanggil Fungsi Lain
     ...
     - batchUpdate (dipanggil oleh: syncProtectionEditors, syncProtectionEditorsBetweenSheet)
     - between (dipanggil oleh: isDate, isYear)
     - capitalize (dipanggil oleh: normalizeFromCamelCase)
     - flat (dipanggil oleh: containOneOf, endWith, includes, isFalsy, isObject, isSame, isTruthy, isTypeOf, lowerThan, max, notSameWith, sameWith, subtract, sum, unique)
     - getColumnLetter (dipanggil oleh: getEventDetail, getRemoteEventDetail, toA1Notation)
     - getFolderIdFromPath (dipanggil oleh: getFileIdByName, getFileIdsIn)
     - getGlobalCache (dipanggil oleh: addToGlobalCache, getFileIdByName, getFolderIdFromPath)
     ...
     - iterate (dipanggil oleh: wrap)
     - lazyWrap (dipanggil oleh: syncProtectionEditors)
     - notSameWith (dipanggil oleh: getFileIdByName)
     - retry (dipanggil oleh: batchUpdate, getFileIdByName, getFileIdsIn, getFolderIdFromPath, syncProtectionEditors, syncProtectionEditorsBetweenSheet, syncRoles)
  ...
  4. Penjelasan Fungsi
     ...
     - getCurrentDate: Decomposer tanggal hari ini ke objek literal komponen numerik {day, date, month, year} dari new Date() runtime.
     ...
     - normalizeFromCamelCase: Konverter camelCase ke human-readable string dengan sisipan spasi sebelum huruf kapital lalu aplikasi mode casing capitalize lowercase uppercase.
